...
const boxgeometry = new THREE.BoxGeometry(1, 1, 1) //main geometry
...
export function BlockEnd({ position = [0, 0, 0] }) {
    ...
    return (
      <group position={position}>
        ...
        <Text font="./AtBriega-Regular.woff" 
          fontSize={0.5}
          scale={ 1 }
            ...
            if(obstacle.current)
                {
                    ...
                    rotation.setFromEuler(new THREE.Euler(0, time * speed, 0)) // EJE Y EN ESTE CASO CON RANDOM SPEED
...
export function BlockLimbo({ position = [0, 0, 0] }) {
    ...
    return (
      <group position={position}>
          ...
          {/* SECOND MESH TRAP NUMBER 2 LIMBO */}
          {/* floor */}
        <mesh geometry={boxgeometry} material={floor2Material} position={[0, -0.1, 0]} scale={[4, 0.2, 4]} receiveShadow />
            {/* trap N 2 */}
        <RigidBody ref={obstacle} type="kinematicPosition" position={[0, 0.3, 0]} restitution={0.2} friction={0}>
